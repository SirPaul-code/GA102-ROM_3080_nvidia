...
function Get-MinCudaForVS([int]$VsMajor) {
    switch ($VsMajor) {
        18 { return [version]"13.2" } # NVIDIA added official VS 2026 / MSVC 195x support in CUDA 13.2.
        17 { return [version]"11.8" } # Known-good baseline with official VS 2022 support.
        16 { return [version]"11.2" } # CUDA 11.2 officially supports VS 2019.
...
if (-not $cuda) {
    ...
    if ($LASTEXITCODE -ne 0) {
        # If an older package is registered as installed, try upgrade as a second path.
...
# A build directory generated by another VS/CUDA pairing can poison CMake's cache.
