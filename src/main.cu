...
static void print_device(const cudaDeviceProp& p) {
    double bw = 2.0 * (double)p.memoryClockRate * 1000.0 * ((double)p.memoryBusWidth / 8.0) / 1e9;
    std::cout << "\n=== Device ===\n"
              << "name              : " << p.name << "\n"
...
__global__ void gemv_i8_dp4a(const int8_t* __restrict__ w,
    ...
    for (; i < K; ++i) acc += (int)wr[i] * (int)x[i];
    y[row] = acc;
}
...
// Exact ternary dot product using two 1-bit weight masks and 8 activation bitplanes.
// signed int8 reconstruction: b0 + 2b1 + ... + 64b6 - 128b7.
__global__ void gemv_bitplane_popc(const uint32_t* __restrict__ pos,
    ...
    for (int bit = 0; bit < 8; ++bit) {
        ...
        int scale = (bit == 7) ? -128 : (1 << bit);
        acc += count * scale;
    }
    y[row] = acc;
}

__global__ void stream_w2(const uint8_t* __restrict__ w, uint64_t bytes,
                          uint64_t* __restrict__ sink) {
    uint64_t i = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t stride = (uint64_t)gridDim.x * blockDim.x;
    uint64_t sum = 0;
    for (; i < bytes; i += stride) sum += w[i];
    if ((threadIdx.x & 31) == 0) atomicAdd((unsigned long long*)sink, (unsigned long long)sum);
}

// Raw Ampere binary Tensor Core probe. One warp repeatedly executes
// mma.sync.aligned.m16n8k256.row.col.s32.b1.b1.s32.and.popc.
// This is deliberately a microarchitectural probe, not the final GEMV mapping.
...
__global__ void bmma_probe(const uint32_t* __restrict__ A,
    ...
    if (lane < 4) out[warp * 4 + lane] = lane == 0 ? c0 : lane == 1 ? c1 : lane == 2 ? c2 : c3;
}
#endif
...
static void cpu_reference(const std::vector<int8_t>& w,
    ...
    for (int r = 0; r < M; ++r) {
        int32_t acc = 0;
        for (int c = 0; c < K; ++c) acc += (int32_t)w[(size_t)r * K + c] * (int32_t)x[c];
        y[r] = acc;
    }
}
...
static void run_ternary(int M, int K, int iters) {
    ...
    for (int r = 0; r < M; ++r) {
        for (int c = 0; c < K; ++c) {
            ...
            hw2[(size_t)r * packed_per_row + (c >> 2)] |= (uint8_t)(code << (2 * (c & 3)));
        }
    }

    int words = (K + 31) / 32;
    std::vector<uint32_t> hpos((size_t)M * words, 0), hneg((size_t)M * words, 0);
    ...
    for (int c = 0; c < K; ++c) {
        ...
        for (int b = 0; b < 8; ++b)
            if ((u >> b) & 1u) hxb[(size_t)b * words + (c >> 5)] |= 1u << (c & 31);
    }
    ...
    auto check_and_time = [&](const char* name, const std::function<void()>& launch, double weight_bytes) {
        launch();
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    check_and_time("I8 DP4A", [&]{ gemv_i8_dp4a<<<blocks, threads>>>(dw, dx, dy, M, K); }, (double)hw.size());
    check_and_time("W2 decode+DP4A", [&]{ gemv_w2_dp4a<<<blocks, threads>>>(dw2, dx, dy, M, K); }, (double)hw2.size());
    check_and_time("W1x2 POPC", [&]{ gemv_bitplane_popc<<<blocks, threads>>>(dpos, dneg, dxb, dy, M, words); }, (double)(hpos.size() + hneg.size()) * sizeof(uint32_t));

    uint64_t* dsink = nullptr;
    CUDA_CHECK(cudaMalloc(&dsink, sizeof(uint64_t)));
    CUDA_CHECK(cudaMemset(dsink, 0, sizeof(uint64_t)));
    int sblocks = 4096;
    float stream_ms = time_ms([&]{ stream_w2<<<sblocks, 256>>>(dw2, (uint64_t)hw2.size(), dsink); }, 5, iters);
    std::cout << std::left << std::setw(20) << "W2 raw stream"
              << "                  " << std::right << std::fixed << std::setprecision(3) << stream_ms << " ms"
              << "  " << std::setprecision(1) << ((double)hw2.size() / (stream_ms * 1e6)) << " GB/s\n";
...
static void run_bmma(const cudaDeviceProp& p, int loops, int iters) {
    ...
    // host pass never sees this branch
#endif
    float ms = time_ms([&]{
        bmma_probe<<<blocks, threads>>>(da, db, dout, loops);
    }, 5, iters);
